...
SvgParams::SvgParams(const Json::Dict& settings) {
    ...
    const auto& vec_with_strings = settings.at("layers").AsArray();
    for (const auto& str : vec_with_strings) {
        this->layers.push_back(str.AsString());
    }
}
